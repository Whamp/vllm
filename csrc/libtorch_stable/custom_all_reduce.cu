...
/**
 * int8 blockwise-compressed allreduce (task #35).
 *
 * Quantizes `inp` (bf16) into the IPC-registered `_reg_buffer` -- displacing
 * the d2d copy the eager path already pays -- then runs the compressed two-shot
 * and writes the reduced payload into `out_q` / `out_s`, still quantized. The
 * consumer (mhc_post) dequantizes, which is what makes the wire saving reach
 * the wall instead of being handed back by a bf16 phase 2.
 */
void all_reduce_int8(fptr_t _fa, torch::stable::Tensor& inp,
  ...
  // Scales follow the codes, 16B-aligned so the QBlock vector stores stay
  // aligned. The same offset applies to the peer buffers and the result.
  ...
  // The tmp buffer holds one shard: the largest rank's block count, then its
  // scales, with the same 16B alignment rule.
