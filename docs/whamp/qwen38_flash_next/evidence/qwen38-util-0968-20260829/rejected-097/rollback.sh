...
set -euo pipefail
runtime=/home/will/inference/runtime/qwen38-util-097
current_runtime=/home/will/inference/runtime/qwen38-rope-bound
candidate=qwen38-util-097-candidate
...
docker compose -p qwen38-util-097 --profile qwen38-flash-next -f "$runtime/candidate.yml" down --remove-orphans || true
sudo -n swapoff -a
...
if docker inspect "$production" >/dev/null 2>&1; then
  ...
  docker compose -p qwen38-rope-prod --profile qwen38-flash-next -f "$current_runtime/production.yml" up -d
fi
...
curl -fsS http://127.0.0.1:30001/v1/models | grep -q 'qwen3.8-flash-next-intel-autoround-w4a16'
