...
- `native-dp4a-*`: one-launch DP4A gate, source, and result.
- `native-mma-*`: activation-quantize plus SM86 IMMA gate, source, header, and result.
...
The first Triton hyperconnection and QSA candidates passed numerical and deterministic CUDA-Graph checks and failed their unchanged performance thresholds. The native hyperconnection candidates preserved the real-weight normalized-error and cosine gates, but failed the generated absolute-error bound and the unchanged performance threshold. None was used in a full-model launch.
