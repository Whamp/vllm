...
def simulate(seed: int) -> dict:
    ...
    key_codes, key_scale = symmetric_int8(transformed_key)
    value_codes, value_scale, value_zero_point = asymmetric_int4(transformed_value)

    key_reconstructed = key_codes * key_scale[:, None]
...
def main() -> None:
    ...
    result = {
        "schema_version": 1,
        "cache_format": {
            "key": "symmetric INT8 per token and head with FP32 scale",
            "value": "asymmetric packed INT4 per token and head with FP32 scale/ZP",
            "main_cache_bytes_per_token_per_layer": 392,
            "complete_qsa_bytes_per_token_per_rank": 5472,
        },
        "geometry": {
            "head_dim": HEAD_DIM,
