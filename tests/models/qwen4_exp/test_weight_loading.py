...
from vllm.models.qwen4_exp.common.weight_loader import Qwen4ExpWeightsLoader
...
def test_qwen4_exp_weight_loader_filters_only_declared_mapped_names() -> None:
    loader = Qwen4ExpWeightsLoader(
        torch.nn.Linear(2, 2),
        skip_prefixes=["visual."],
        skip_substrings=["mtp."],
    )

    assert loader._can_skip("visual.blocks.0.weight")
    assert loader._can_skip("model.mtp.layers.0.weight")
    assert not loader._can_skip("model.layers.0.weight")
