...
@pytest.mark.parametrize(
    ("group_size", "down_bits"),
    [(128, 2), (256, 2), (512, 2), (128, 4), (256, 4), (512, 4)],
    ids=["w2-g128", "w2-g256", "w2-g512", "w4-g128", "w4-g256", "w4-g512"],
)
def test_humming_wna16_grouped_indexed_numerical_oracle(group_size, down_bits):
    from vllm.forward_context import set_forward_context
    from vllm.utils import humming
    ...
    weight_schemas = {
        ...
        for sublayer_name, bits in (("w13", 2), ("w2", down_bits))
    }
    ...
    layer = experts.layer
    for sublayer_name, expected_dtype in (
        ("w13", humming.dtypes.uint2),
        ("w2", humming.dtypes.uint2 if down_bits == 2 else humming.dtypes.uint4),
    ):
        meta = layer.humming_metas[sublayer_name]
        assert meta.a_dtype == humming.dtypes.bfloat16
        assert meta.b_dtype == expected_dtype
        assert meta.weight_scale_group_size == group_size
