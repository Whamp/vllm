...
def get_kv_cache_physical_allocation_bytes(
    ...
    allocation_sizes = {tensor.size for tensor in kv_cache_config.kv_cache_tensors}
    if not allocation_sizes:
        return 0
    assert len(allocation_sizes) == 1, (
        "Standardized KV cache tensors must share one backing allocation size."
    )
    return allocation_sizes.pop()
...
def _build_kv_cache_spec_allocation_report(
    ...
    storage_block_size = getattr(spec, "storage_block_size", spec.num_states)
...
def build_kv_cache_allocation_report(
    ...
    layout = kv_cache_config.kv_cache_layout or "standardized"
    packed = len(kv_cache_config.kv_cache_tensors) > 1
