...
class KVQuantMode(IntEnum):
    ...
    FP4_DS_MLA = 10  # DeepSeek V4 E2M1 NoPE + BF16 RoPE paged layout
...
class MLAAttentionSpec(FullAttentionSpec):
    ...
    def storage_block_size(self) -> int:
        """Compatibility alias for the standardized stored-state count."""
        return self.num_states

    @property
    def unpadded_page_size_bytes(self) -> int:
        if self.physical_row_bytes is not None:
            return self.num_states * self.physical_row_bytes
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        assert (
            ...
            "quantization method, tokens per state, model version, physical "
            "row size, semantic width, and non-causal decode mode."
        )
        merged_spec = cls(
            block_size=specs[0].block_size,
            num_kv_heads=specs[0].num_kv_heads,
...
class SlidingWindowMLASpec(SlidingWindowSpec):
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        assert (
            ...
            "All attention layers in the same KV cache group must use the same "
            "quantization method, tokens per state, model version, physical "
            "row size, sliding window size, and retained token count."
        )
        return cls(
            block_size=specs[0].block_size,
            num_kv_heads=specs[0].num_kv_heads,
