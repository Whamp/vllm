...
def _sparse_attn_num_warps(compress_ratio: int, overlap: bool) -> int:
    """Warps for the head=512 fused compressor, sized to the rows a program
    reduces: ``(1 + overlap) * compress_ratio``. A100-measured anchors are
    rows=8 -> 4 warps and rows=128 -> 16 (a flat 4 spills on the 128-row
    tile); the clamp keeps every ratio inside the measured range."""
    rows = (2 if overlap else 1) * compress_ratio
    return max(4, min(16, rows // 2))
